let buttonn = document.querySelector(".like-button");
...
function generateClones(buttonn) {
    let clones = randomInt(2, 4);
    for (let it = 1; it <= clones; it++) {
        let clone = buttonn.querySelector("svg").cloneNode(true),
            size = randomInt(5, 16);
        buttonn.appendChild(clone);
        ...
        let removeClassTimeout = setTimeout(function () {
            buttonn.classList.remove("animated")
        }, 600);
    }
}
